...
function ProfileDropdown() {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-green-600 text-base font-medium text-white">
          {user.initials}
        </span>
      </button>
...
const navItems = [
  ...
  { name: "Ciclo das Sementes", href: "/ciclo-sementes" },
  { name: "Relatórios", href: "/relatorios" },
  { name: "Configurações", href: "/configuracoes" },
];

interface HeaderProps {
  activeItem: string; // Nome do item de navegação ativo
}
 ...
 */
function Header({ activeItem }: HeaderProps) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-16">
          ...
          <div className="flex-shrink-0 flex items-center mr-10">
            ...
            <span className="text-xl font-semibold text-green-700">Ecosy</span>
          </div>

          {/* Links de Navegação */}
          <nav className="hidden sm:flex sm:space-x-8 h-full">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className={`inline-flex items-center px-1 pt-1 text-sm font-medium transition-colors ${
                  item.name === activeItem
                    ? "border-b-4 border-green-600 text-gray-900 font-semibold"
